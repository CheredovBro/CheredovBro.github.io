// const nameList = [
//     {link: 'https://google.com', age: 12, color: 'red', name: 'Svyryd', img: `data:image/jpeg;base64,`},
//     {link: 'https://google.com', age: 28, color: 'green', name: 'Opanas', img: `data:image/jpeg;base64,`},
//     {link: 'https://google.com', age: 67, color: 'blue', name: 'Nestor Ivanovych Makhno', img: `data:image/jpeg;base64,`},
//     {link: 'https://google.com', age: 28, color: 'orange', name: 'Bogdan', img: `data:image/jpeg;base64,`},
//     {link: 'https://google.com', age: 23, color: 'pink', name: 'Ostap', img: `data:image/jpeg;base64,`},
//     {link: 'https://google.com', age: 54, color: 'coral', name:'Nazar', img: `data:image/jpeg;base64,`},
// ];

// const app = document.querySelector('#app');
// if(app){
    
//     let html = '';
//     nameList.forEach( (obj, i)=> {
//         if(!(i % 2)){
//             html += `<li class = "names-item" style = "background: ${obj.color}">
//             <img src="${obj.img}"/>
//             <a href = "${obj.link}" target = "__blanck">href</a>
//             ${obj.name}; ${obj.age > 16 ? 'too old' : obj.age};</li>`
//         }
//     })


//     html = `<ul class = "names">${html}</ul>`;
//     app.innerHTML = html;

    
// }
////////////////////////////////////////////////////

// const colomoyskyConfigs = [
//     {
//         animation: 100,
//         speed: .5,
//         height: '100px'
//     },
//     {
//         animation: 10,
//         speed: 1,
//         height: '100px'
//     },
//     {
//         animation: 30,
//         speed: 3,
//         height: '100px'
//     },
//     {
//         animation: 40,
//         speed: 5,
//         height: '100px'
//     },
//     {
//         speed: .8,
//         height: '100px',
//         animation: 20
//     },
// ]

// function render(selector, configs){
// /* <div class="iv_jumping"></div> */
//     const ap = document.querySelector('#ap');

//     let html = configs.map(e => 
//          `<div
//                     style = "animation-duration: ${e.speed}s;
//                     animation-duration: ${e.animation};
//                     "
//                     class="iv_jumping">
//                 </div>`
//     ).join('');
//     ap.innerHTML = html;
// }

// render('#ap', colomoyskyConfigs);

///////////////////////////////////////////////////////////

// const arr = Array(100).fill(' ');

// function randomNum(){
//     return Math.round(Math.random() * 255);
// }

// function render(selector, list){
//     const ap = document.querySelector(selector);
//     const html = arr.map(e => `
//     <div
//         style ="
//         width: 50px;
//         height: 50px;
//         background: rgb(${randomNum()}, ${randomNum()}, ${randomNum()});
//         "
//     ></div>`
//         ).join('');

    

//     ap.innerHTML = html;

// }

// render('#ap', arr)
// function example(obj){
//     console.log('a', obj.a)
//     console.log('a', obj.b)
//     console.log('a', obj.c)
//     console.log('a', obj.d)
//     console.log('a', obj.e)
//     console.log('f', obj.f)
// }example({
//     a: 1,
//     f: 6
// })

// function second(name, sName, age = 'not set'){
//     const person = `
//     name: ${name}
//     second name: ${sName}
//     age: ${age}`;

//     console.log(person)
// }

// second('semen', 'petrenko')

// function greeting(name = 'semen'){
//     const greet = `Hello ${name}`;

//     document.body.innerHTML = `
//     <h1>${greet}</h1>
//     `
// }

// const exm = (a, b, ...rest) =>{
//     console.log(rest)
// }


// const person = (name = 'gest', ...rest) =>{

//     const greeting =`
//     <h1> hello ${name}!</h1
//     ${rest.length ? `<ul>${rest.map(e => `<li>${e}</li>`).join('')}</ul>` : ''}`

//    document.body.innerHTML = greeting
// }
// person ('Max', 'Cheredov', '+3336546442');

// function example(){
//     let variable = 132

//     return function(){
//         console.log(variable)
//     }
// }
// let res = example()

// console.dir(res)

// function constr(rootNumber){
//     return function(num){
//         return num + rootNumber;
//     }
// }

// const sumWith2 = constr(2)
// console.dir(sumWith2)


// const sumWith5 = constr(5)
// console.dir(sumWith5)

// const fraseConstructor = frase => name =>  `${frase}, ${name}!`
    

// const sayHallo = fraseConstructor('hello')
// const saygoodby = fraseConstructor('Goodby')

// console.log(sayHallo('Ivan'))


// function circleSquareConstructor(){
//     const PI = 3.14;

//     return R => PI * R * R;
// }

// const circleSquare = circleSquareConstructor()

/////////////////////////////////////////////////////////////////

// const rundom = () => Math.round(Math.random()*255);
// let timer =  setInterval(render, 50);
// function render(selector = '#ap'){
//     let html = '';

//     for(let i = 0; i<500; i++){
//         html += `<div
//             style = "
//                 width: 80px;
//                 height: 80px;
//                 background: rgb(${rundom()}, ${rundom()}, ${rundom()})
//             "
//         ></div>`
//     }

//    const element = document.querySelector(selector);
//    if(element){
//        element.innerHTML = html;
//    }else{
//        console.log('no element')
//        clearInterval(timer)
//    }
// }


////////////////////////////////////////////////////////////////////////////////////////////


// function Example(){
//     this.field1 = 123;
//     this.field2 = 'qwwqwqe';
//     this.log = function(){
//         console.log(this);
//     }
// }

// const ex1 = new Example();

// console.log(ex1)

// function Car(selector, color, speed, distance){
//     this.carWidth = 200;
//     this.car = document.querySelector(selector)
//     this.car.style.width = `${this.carWidth}px`;
//     this.color = color;
//     this.speed = speed;
//     this.distance = distance;
//     this.isRun = null;
//     this.run = function(){
//          if(this.isRun) return;
//         this.isRun = setInterval(() => {

//             this.distance+= this.speed;
//             const maxWidth = window.innerWidth;
//             const currentWidth = this.distance + this.carWidth;
//             if(currentWidth >= maxWidth){
//                 this.stop();
//                 this.distance = maxWidth - this.carWidth;
//             }
//             this.move();
//         }, 150)
//     }

//     this.accelerate = function(){
//         while(this.speed <=200){
//         this.speed *= 5;
//         }
//     }

//     this.stop = function(){
//         clearInterval(this.isRun)
//         this.isRun = null;
//         console.log('car is stopped')
//     }

//     this.move = function(){
//         this.car.style.left = `${this.distance}px`
//     }


// }

// const car1 = new Car('#car1', 'red', 10, 1);
// const car2 = new Car('#car2', 'green', 5, 0);

/////////////////////////////////////////////////////////////////

// function Technika(model){
//     this.model = model;
// }

// const bmwTech = new Technika('bmw')
// console.log(bmwTech)

// function MotoCicle(version){
//     this.version = version;
// }

// const makaka = new MotoCicle('makaka')
// console.log(makaka)
///////////////////////////////////////////////////
// function User(){
//     this.greeting = function(){
//         if(!this.name){
//             console.log(`hello guest`);
//             return;
//         }
//         console.log(`Hello ${this.name}`)
//     }
//     this.getFullName = function(){
//         return `${this.name} ${this.lastName}`
//     }
// }
// const commonUser = new User();

// function loggedUser(name, lastName){
//     this.name = name;
//     this.lastName = lastName;
// }

// const Ostap = new loggedUser('Ostap', 'Kryvonis')

// Ostap.__proto__ = commonUser;

// function Pet(name, voice){
//     const blokhy = true;
//     this.name = name;
//     this.voice = voice;
//     this.say = function(){
//         console.log(this.voice);
//     }
//     this.isBlokhy = function(){
//         if(blokhy) console.log('blokht ye')
//         else console.log('blokh nemaye')
//     }
// }

// const dog = new Pet('rex', 'gaw');
// const cat = new Pet('marsic', 'mao');

////////////////////////getter&setter/////////////////////////////

// function User(name, lastName){
//     return{

//     name: name,
//     lastName: lastName,

//         get fullname(){
//             console.log(this);
//             return this.name + ' ' + this.lastName
//         },

//         set fullname(fullname){
//             console.log(fullname);
//             const parts = fullname.split(' ');
//             this.name = parts[0];
//             this.lastName = parts[1];
//         }
//     }
// }

// const user = new User('Ivan', 'Ivanov')

///////////////////////////////////CLASS//////////////////////////////


// class User{
//     name = '';
//     lastName = '';
//     constructor(name, lastName){
//         this.name = name;
//         this.lastName = lastName;
//     }

//     greeting(){
//         console.log('hello, i`m created')
//     }

//     get fullName(){
//         return `${this.name} ${this.lastName}`;
//     }

//     set fullName(fullName){
//         const parts = fullName.split(' ');
//         this.name = parts[0];
//         this.lastName = parts[1];
//     }
// }

// const user = new User('ivan', 'ivanov')

// class Counter {
//     counter = 0;
    
//     increese(){
//         this.counter++;
//     }

//     degreese(){
//         this.counter--;
//     }
// }

// const counter = new Counter();
