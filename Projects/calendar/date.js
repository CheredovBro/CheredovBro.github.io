class Calendar {
    ...
    renderContext(){
        ...
        <div class="context">
            <div class="context_item" id="add">Add</div>
        </div>
        `
    }

    initContextEvents(){
        document.querySelector('.calendar').addEventListener('contextmenu', e => {
            e.preventDefault();
            const context = document.querySelector('.context');
            const target = e.target.closest('.calendar_cell');
            if(!target){
            ...
            <table class="calendar">
                <thead>
                    <tr class="calendar_row">
                        ${
                            this.days.map(day => `<th class="calendar_head-cell">${day}</th>`).join('')
                        }
                    </tr>
                </thead>
                <tbody>
                    ${
                        this.weeksList.map(row => `
                            <tr class="calendar_row">
                                ${row.map(char =>`<td class="calendar_cell" data-date="${char}">
                                <div class = "calendar_cell_inner">
                                    <span class="calendar_date">${char}</span>
                                </div>
                                </td>`).join('')}
                            </tr>
                        `).join('')
